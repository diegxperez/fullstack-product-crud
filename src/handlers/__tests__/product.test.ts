...
describe("GET /api/products/:id", () => {
  ...
  it("should check a valida ID in the URL", async () => {
  ...
  it("get a JSON response for a single product", async () => {
    const product = await request(server).post("/api/products").send({
      name: "Monitor para borrar",
      price: 300,
    });

    const CREATED_ID = product.body.data.id;
    const response = await request(server).get(`/api/products/${CREATED_ID}`);
    expect(response.status).toBe(200);
    expect(response.body).toHaveProperty("data");
  });
});
